...
export function registerBrowserPlugin(api: OpenClawPluginApi) {
  ...
  // Route paths are fixed at registration; use its config snapshot without
  // materializing the live runtime during metadata registration.
  const handoffBasePath = resolveHumanInterventionBasePath(
    resolveHumanInterventionPublicOrigin(api.config) ?? "",
