...
5. Report real blockers:
   ...
   - If the page blocks the task on a CAPTCHA, login/2FA, or another step only the user can complete, and the tool advertises `action="handoff"` for this managed host profile, call it immediately with `target="host"`, the current `profile`, `targetId`, and a short `reason`. The user does not need to ask for handoff or approve sending the link. OpenClaw sends a private link scoped to this handoff and pauses the turn. Do not just say "solve the CAPTCHA" or ask the user to access host Chrome/KVM.
   - Never solve or bypass CAPTCHAs yourself or ask for passwords, verification codes, or CAPTCHA answers in chat. The user completes the step directly in the remote tab. After the handoff resumes the task, inspect a fresh snapshot before acting.
   - If handoff is unavailable for the current profile or the blocked step requires a native OS dialog that the viewer cannot operate, explain the specific blocker and supported manual action. Do not invent a link or switch to an unrelated tab/profile. A failed handoff call does not issue a new link. If an existing handoff is active, explain that it must be completed or cancelled before requesting another; never send a token-free replacement URL.
