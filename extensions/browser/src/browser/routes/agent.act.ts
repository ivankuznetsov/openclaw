...
export function registerBrowserAgentActRoutes(
  ...
  app.post("/act", async (req, res) => {
    ...
    const requestDeadline = isExistingSession
      ...
      : undefined;
    // Human-control input stops when its handoff authority or requester connection ends.
    const authoritySignals = [
      requestDeadline?.signal ?? req.signal,
      req.requester?.signal,
      controlAuthoritySignal,
    ...
    const assertAuthority =
      controlAuthoritySignal || req.requester
        ? () => {
            controlAuthoritySignal?.throwIfAborted();
            if (req.requester?.isCurrent() === false) {
              throw new Error("Browser requester authority is no longer current");
            }
          }
        : undefined;
    const routeReq = requestDeadline ? { ...req, signal: requestDeadline.signal } : req;
    try {
        ...
        req: assertAuthority
          ? {
              ...
              assertCurrent: async (profile) => {
                assertAuthority();
                await requestAssertCurrent?.(profile);
              },
            }
          : routeReq,
        ...
        run: async ({ cdpUrl, tab, signal, resolveTabUrl, assertCurrent }) => {
          ...
          try {
            ...
            const result = await pw.executeActViaPlaywright({
              ...
              targetId: tab.targetId,
              evaluateEnabled,
              includeInputFocus: Boolean(controlAuthoritySignal),
              ...navigationPolicy,
              signal,
