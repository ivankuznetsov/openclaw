...
export const EXISTING_SESSION_LIMITS = {
  act: {
    ...
    dragCoords: "dragCoords is not supported for existing-session browser profiles",
    scrollCoords: "scrollCoords is not supported for existing-session browser profiles",
    insertText:
...
export function getExistingSessionUnsupportedMessage(action: BrowserActRequest): string | null {
  switch (action.kind) {
    ...
    case "scrollCoords":
    case "insertText":
      return EXISTING_SESSION_LIMITS.act[action.kind];
    case "press":
