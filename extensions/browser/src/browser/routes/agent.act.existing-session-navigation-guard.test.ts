...
describe("existing-session interaction navigation guard", () => {
  ...
  it("binds action execution to the supplied human-control authority", async () => {
    ...
    const body: Record<PropertyKey, unknown> = { kind: "clickCoords", x: 10, y: 20 };
    body[browserControlAuthoritySignal] = authority.signal;
    await runAction(body as Record<string, unknown>, null);
    const signal = chromeMcpMocks.clickChromeMcpCoords.mock.calls[0]?.[0].signal;
    expect(signal?.aborted).toBe(false);
    authority.abort();
    expect(signal?.aborted).toBe(true);
  });

  it("binds input to requester liveness independently of request timeout", async () => {
    const connection = new AbortController();
    const timeout = new AbortController();
    const post = (isCurrent: () => boolean) =>
      getActPostHandler(null)?.(
        {
          params: {},
          query: {},
          body: { kind: "clickCoords", x: 1, y: 2 },
          signal: timeout.signal,
          requester: { signal: connection.signal, isCurrent },
        },
        createBrowserRouteResponse().res,
      );
    // A revoked requester is rejected before its socket closes.
    await expect(post(() => false)).rejects.toThrow("requester authority");
