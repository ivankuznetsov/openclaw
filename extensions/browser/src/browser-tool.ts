  ...
  return {
    ...
    execute: async (toolCallId, args, signal) => {
      ...
      if (action === "handoff") {
        ...
        if (dashboardName) {
          // Dashboard tabs are shared with session writers; handoff needs an agent-owned tab.
          throw new Error("Human browser handoff is unavailable for dashboard tabs");
        }
        target = "host";
      }
