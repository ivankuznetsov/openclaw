...
Open the link to enter the browser viewer. When you take control, the bound tab resizes to the available viewing area so responsive websites can use the phone screen. Opening the keyboard does not resize the remote page again. The page supports taps, drags, page scrolling, local zoom, and direct keyboard input. On a touch screen, tap a text field to select it, then tap it again when prompted to open the keyboard. Swipe with one finger anywhere in the viewer to scroll the remote page. Pinch with two fingers to zoom locally, and move both fingers to pan the zoomed view. Mouse dragging still moves page elements on desktop. Mobile zoom buttons are hidden, and **Task details** contains only the task instructions. It does not expose browser navigation commands, evaluation, cookies, files, shell access, or other browser profiles. Clicking links or typing into the page can still navigate within the selected tab; access is scoped to the tab, not to one website.

- **Done — continue agent** revokes human input and schedules the original session to inspect fresh page state before continuing. Once continuation is queued, the viewer closes automatically where the browser permits it. Otherwise, a brief status remains; no second confirmation is required.
- **Cancel handoff** ends the handoff without resuming the task.

Closing or backgrounding the page leaves the task paused. A controller lease also expires after a disconnect. Return to the same browser tab to take control again. After reconnecting, you may need to wait for the previous control lease to expire. Delayed input or release requests from an earlier claim cannot affect the new controller. An already-redeemed link cannot be used to authorize another browser; if you close the tab and lose its session storage, the existing handoff must end before a new one can be created. An authenticated administrator can cancel it, or you can wait for it to expire (30 minutes by default).
