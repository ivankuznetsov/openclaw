...
An owner-authorized iMessage DM can receive an HTTPS **Open browser** link when an agent pauses a managed browser tab for live human verification. The link opens the shared mobile viewer using a one-use credential limited to that handoff. No Gateway sign-in is required, and completion returns to the originating iMessage route. See [Human browser intervention](/tools/browser-human-intervention).
